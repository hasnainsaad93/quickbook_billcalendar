...
namespace BillCalend.Util
{
    ...
    public class EventManager
    {
        ...
        private string key;
        static public bool calenFormLoaded = false;
        ...
        private DataTable generateData()
        {
            ...
            // fill calendar component with events from db
            DataTable dt = new DataTable();
            ...
            if (calenFormLoaded)
            {
                ...
                List<BillCalend.Models.Event> lEvents = dbAccessor.GetEvents(username);
                foreach (BillCalend.Models.Event ev in lEvents)
                {
                    DataRow dr;
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [event] WHERE [start] between @start and @end", ConfigurationManager.ConnectionStrings["daypilot"].ConnectionString);
        ...
        internal void EventCreate(DateTime start, DateTime end, string text, string resource, string recurrenceJson)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["daypilot"].ConnectionString))
            {
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO [events] ([start], [end], [username]) VALUES (@start, @end, @name); ", con);  // SELECT SCOPE_IDENTITY();
                cmd.Parameters.AddWithValue("start", start);
                cmd.Parameters.AddWithValue("end", end);
